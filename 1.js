...
let arrChecboxText = arrCheckbox.map(
  (el) => Array.from(el).map((el) => el.innerText) // Получаем массив, в который маппим все значения, которые содержаться отдельные массивы всеъ дементов с страницы из таблицы
);
...
Array.from(check).forEach((item) =>
  item.addEventListener('click', () => workCheckedElem(item))
);
...
function workCheckedElem(item) {
  console.log(item);
  if (item.checked === true) {
    arrChecboxText.forEach((el, ind) => {
      if (item.id == ind && item.value == el[1]) {
        el.splice(0, 1, true);
        el[0] == true ? (textareaResultDataQuery.value += `${el}\n\n`) : null;
      }
    ...
    arrChecboxText.forEach((el, ind) => {
      if (item.id == ind && item.value == el[1]) {
        el.splice(0, 1, false);
        el[0] == false ? (textareaResultDataQuery.value = el) : null;
      }
